...
const BookForm: FC<IBookForm> = ({book, onSubmit}) => {
    ...
    return <div>
        <Form form={form} labelCol={{span: 4}} onFinish={onFinish} >
            <Form.Item label="Title" name="title" rules={[{required: true}]} initialValue={book?.title}>
                <Input disabled={!!book?.id} />
            </Form.Item>
            <Form.Item label="Author" name="author" rules={[{required: true}]} initialValue={book?.author}>
                <Input />
            </Form.Item>
